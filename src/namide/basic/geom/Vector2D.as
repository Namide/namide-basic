...
{	
	/**
	 * The DVector2D is a 2 dimensional vector, inspired by playchilla.com
	 * 
	...
	public class Vector2D
	{
		...
		public function setByVector2D( vect:Vector2D ):Vector2D
		{
			_x = vect._x;
			_y = vect._y;
			return this;
		 ...
		 */
		public function add(pos:Vector2D):Vector2D { return new Vector2D(_x + pos._x, _y + pos._y); }
		 ...
		 */
		public function sub(pos:Vector2D):Vector2D { return new Vector2D(_x - pos._x, _y - pos._y); }
		 ...
		 */
		public function multiply(vec:Vector2D):Vector2D { return new Vector2D(_x * vec._x, _y * vec._y); }
		 ...
		 */
		public function divide(vec:Vector2D):Vector2D { return new Vector2D(_x / vec._x, _y / vec._y); }
		 ...
		 */
		public function distanceSqrt(vec:Vector2D):Number
		{
			const xd:Number = _x - vec._x;
			const yd:Number = _y - vec._y;
			return xd * xd + yd * yd;
		 ...
		 */
		public function equals(vec:Vector2D):Boolean { return _x == vec._x && _y == vec._y; }
		 ...
		 */
		public function isNear(vec:Vector2D):Boolean
		{
		 ...
		 */
		public function isWithin(vec:Vector2D, epsilon:Number):Boolean { return distanceSqrt(vec) < epsilon * epsilon; }
		 ...
		 */
		public function crossDeterminant(vec:Vector2D):Number { return _x * vec._y - _y * vec._x; }
		 ...
		 * Spinor rotation ?
		 */
		public function rotateSpinor(vec:Vector2D):Vector2D { return new Vector2D(_x * vec._x - _y * vec._y, _x * vec._y + _y * vec._x); }
		
		/**
		 * Spinor rotation ?
		 */
		public function spinorBetween(vec:Vector2D):Vector2D
		{
		 ...
		 * Lerp
		 */
		public function interpolateTo(to:Vector2D, t:Number):Vector2D { return new Vector2D(_x + t * (to._x - _x), _y + t * (to._y - _y)); }
		 ...
		 * @return
		 */
		public function sphericalInterpolateTo(vec:Vector2D, t:Number):Vector2D
		{
		...
		public function copy(vec:Vector2D):Vector2D
		{
			_x = vec.x;
			_y = vec.y;
			return this;
		 ...
		 */
		public function project( vect:Vector2D ):Vector2D
		{
		 ...
		 * Add
		 */
		public function addSelf(vec:Vector2D):Vector2D
		{
			_x += vec.x;
			_y += vec.y;
			return this;
		 ...
		 * Sub
		 */
		public function subSelf(vec:Vector2D):Vector2D
		{
			_x -= vec.x;
			_y -= vec.y;
			return this;
		 ...
		 * lerp
		 */
		public function interpolateToSelf(to:Vector2D, t:Number):Vector2D
		{
			_x = _x + t * (to.x - _x);
			_y = _y + t * (to.y - _y);
			return this;
		 ...
		 * Project the vector into a vector
		 */
		public function projectSelf( vect:Vector2D ):Vector2D
		{
			const s:Number = dot( vect ) / ( vect.length() * vect.length() );
			return scaleSelf( s );
		}
		...
		{
			return getRadsBetween( A, O, B ) * Vector2D._RadsToDeg;
		}
