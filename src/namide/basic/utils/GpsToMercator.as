package namide.basic.utils 
{
	...
	public class GpsToMercator 
	{
		...
		//protected var minMercator:Point;
		//protected var maxMercator:Point;
		protected var proportionalMercatorX:Equation2D;
		protected var proportionalMercatorY:Equation2D;
		
		//protected var centerMercator:Point;
											   ...
											   minMercX:Number = 0, maxMercX:Number = 1, minMercY:Number = 0, maxMercY:Number = 1 ):GpsToMercator
		{
			...
			/*const limitMinGpsY:Number = 85;
			const limitMaxGpsY:Number = -85;
			const limitMinGpsX:Number = -180;
			const limitMaxGpsX:Number = 180;
			
			
			var minRealUV:Point = convertUV( limitMinGpsX, limitMinGpsY );
			var maxRealUV:Point = convertUV( limitMaxGpsX, limitMaxGpsY );*/
			...
			//trace(centerMercator);
			
			/*trace( this.minGpsX, this.minGpsY, "minDisplaceUV:", minDisplaceUV);
			trace( this.maxGpsX, this.maxGpsY, "maxDisplaceUV:", maxDisplaceUV);
			
			trace( this.minMercX, this.minMercY, "minDisplaceMerc:", minDisplaceMerc);
			trace( this.maxMercX, this.maxMercY, "maxDisplaceMerc:", maxDisplaceMerc);*/
			...
			/*trace("proportionalMercatorX:", proportionalMercatorX);
			trace("proportionalMercatorY:", proportionalMercatorY);*/
			
			return this;
		}
		
		/*protected function searchCenterMercator():Point
		{
			var centerGPS:Point = new Point( (maxGpsX + minGpsX) * 0.5, (maxGpsY + minGpsY) * 0.5 );
			var centerMerc:Point = convertProportionnal( centerGPS.x, centerGPS.y, false );
		...
		public function convertProportionnal( gpsY:Number, gpsX:Number ):Point
		{
			...
			/*var pos:Point = convertUV(gpsY, gpsX);		// { 0 ; 0 } -> { 1 ; 1 }
			
			pos.x = pos.x - 0.5;
			pos.y = pos.y - 0.5;
			
			const scaleW:Number = (limitMaxGpsX - limitMinGpsX) * (maxMercX - minMercX) / (maxGpsX - minGpsX);
			const scaleH:Number = (limitMaxGpsY - limitMinGpsY) * (maxMercY - minMercY) / (maxGpsY - minGpsY);
		...
		protected function gpsToMercator(lon:Number, lat:Number):Point //lat lon to mercator
		{
			//lat, lon in rad
		...
		protected function mercatorToGps(x:Number, y:Number):Point //mercator to lat lon
		{
